...
import { GeneratePromptRequestSchema, Question, UserAnswer } from '@/types/schemas';
...
function getDomainSystemPrompt(domain: string, locale: string = 'vi'): string {
  ...
  let domainRules = '';
  if (d.includes('hình ảnh') || d.includes('image')) {
...
Your goal is to transform the user's initial seed idea and answered clarification questions into a master-class, production-ready final prompt.
${langDirective}
...
- Include a brief section explaining why key design choices were made based on the user's answers.`;
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { seed, domain, questions, answers, additionalContext, locale = 'vi' } = parseResult.data;

    const systemPrompt = getDomainSystemPrompt(domain, locale);
    const userPrompt = formatUserContext(seed, domain, questions, answers, additionalContext);
