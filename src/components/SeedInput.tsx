    ...
    <div className="w-full space-y-2.5">
      <div className="flex items-center justify-between">
        <label
          ...
          className="text-[11px] font-bold uppercase tracking-wider text-[#8E8377] dark:text-[#7E7368]"
        >
          {t.seedInput.label}
        </label>
        <span
      ...
      <div
        ...
        onDrop={handleDrop}
        className={`relative rounded-lg border transition-all duration-150 p-2.5 shadow-2xs ${
          isDragging
      ...
      >
        <textarea
          ...
          className="w-full min-h-[160px] md:min-h-[200px] resize-y rounded bg-transparent p-1.5 text-sm leading-relaxed text-[#2B2520] placeholder:text-[#8E8377] focus:outline-hidden dark:text-[#EDE5DC] dark:placeholder:text-[#7E7368]"
        />

        {/* Drop zone overlay indicator */}
        {isDragging && (
          <div className="absolute inset-0 bg-[#FDF6F0]/95 dark:bg-[#282320]/95 rounded-lg flex flex-col items-center justify-center pointer-events-none z-10">
            <UploadCloud className="h-9 w-9 text-[#C15F3D] dark:text-[#DA7756] animate-bounce" />
            <p className="mt-1.5 text-xs font-bold text-[#C15F3D] dark:text-[#DA7756]">
        ...
        {attachments.length > 0 && (
          <div className="flex flex-wrap gap-1.5 p-2 border-t border-[#E6DFD3]/60 dark:border-[#38312C]/60">
            {attachments.map((att) => (
              <div
                key={att.id}
                className="group relative flex items-center gap-1.5 rounded border border-[#E6DFD3] bg-[#F5F0E6]/80 px-2 py-1 text-xs text-[#2B2520] dark:border-[#38312C] dark:bg-[#1F1A18] dark:text-[#EDE5DC]"
              >
                {att.dataUrl ? (
                  ...
                  <img
                    src={att.dataUrl}
                    alt={att.name}
                    className="h-5 w-5 rounded object-cover border border-[#E6DFD3] dark:border-[#38312C]"
                  />
                ) : (
                  <FileText className="h-3.5 w-3.5 text-[#8E8377] dark:text-[#7E7368]" />
                )}
                <span className="max-w-[100px] truncate text-[11px] font-medium" title={att.name}>
                  {att.name}
                </span>
                ...
                <button
                  ...
                  disabled={disabled || isLoading || isDeconstructing}
                  className="rounded p-0.5 text-[#8E8377] hover:bg-[#E6DFD3] hover:text-[#C15F3D] dark:text-[#7E7368] dark:hover:bg-[#38312C] dark:hover:text-[#DA7756] cursor-pointer"
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 border-t border-[#E6DFD3]/60 pt-2 px-1 dark:border-[#38312C]/60">
          <div className="flex items-center gap-1.5">
            ...
            {attachments.length > 0 && onDeconstruct && (
              <button
                ...
                disabled={disabled || isLoading || isDeconstructing}
                className="inline-flex items-center gap-1 rounded border border-[#C15F3D]/30 bg-[#FDF6F0] px-2.5 py-1 text-xs font-medium text-[#C15F3D] transition hover:bg-[#C15F3D] hover:text-white disabled:opacity-50 dark:border-[#DA7756]/30 dark:bg-[#33231D] dark:text-[#DA7756] dark:hover:bg-[#DA7756] dark:hover:text-white cursor-pointer"
          ...
          <div className="flex items-center gap-2">
            <span className="hidden sm:inline text-[11px] text-[#8E8377] dark:text-[#7E7368]">
